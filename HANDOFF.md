...
| Share/import links | 🟡 Code complete — SHARE sits behind the parental gate (owner decision 2026-09-04) and exports the HTTPS import link; opening one creates a new local event UUID; cross-device share still needs manual QA |
...
- **Share/import is copy-based, not collaboration.** The detail screen shares a versioned `sleeps://import?d=…` payload containing title, date/time, icon, and color. Opening it inserts a new `CountdownEvent`, so imports never reuse the sender's UUID. Existing-event widget links remain `sleeps://event/<uuid>` and do not import. **SHARE is gated** (owner decision 2026-09-04): the button shows `ParentalGateView` first, and the share sheet (a `UIActivityViewController` wrapper, since `ShareLink` can't be presented programmatically) opens from the gate sheet's `onDismiss` — not from `onPass`, because a sheet set while the previous sheet is still animating out is silently dropped by SwiftUI. `ParentalGate`/`ParentalGateView` live in `Countdown/Views/ParentalGate.swift`, shared with Settings.
...
- **Live Activities are local-only.** ActivityKit starts them for exactly one sleep away or a timed event in its final 24 hours, with Lock Screen and Dynamic Island presentations. No push token is requested. Event taps use the existing local-event deep link (`widgetURL` is set on both the Lock Screen view and the Dynamic Island, not just one). Because there's no push, the activity itself only *ends* on the next foreground/background/edit-triggered `LiveActivityManager.sync` — by design, not a bug — but in the meantime `SleepsLiveActivity`'s views recompute `CountdownPhase` from `state.eventDate`/`hasTime` live, so once the moment arrives they show the same "TODAY!"/"IT'S TIME!" copy as `EventDetailView` instead of freezing on stale "1". `LiveActivityManager` also sets `ActivityContent.staleDate` to the event day's `startOfDay` for untimed "1 more sleep" activities, so the system can visually dim them once that content is known to be outdated (timed/ticking activities stay fresh on their own via `Text(timerInterval:)`, so they don't set one).
...
2. ~~App Store Connect record~~ — **exists; v1.0 is live** (as "Sleeps - how many?", category Lifestyle). Create a **1.0.1** version on the existing record instead. **Owner decision 2026-09-04: staying Lifestyle** — the Kids-category move (age band 6–8) is deliberately deferred, because entering Kids stays available on any future update while leaving it is restricted, Kids review would tax every release, and the family-wide audience is wider than a 6–8 shelf label. The parental gate ships anyway, so the app is already Kids-compliant if the call ever flips. **⚠️ ASC trap (bit us 2026-09-05):** "Made for Kids" is NOT set via the Category picker — it hides in App Information → Age Rating → "Age Categories and Override". Selecting "Made for Kids" there enrolls the app in full Kids-Category review (the 1.3 questionnaire) even while Category reads Lifestyle; "Not Applicable" is the off switch, and per Apple's own fine print the designation only becomes binding once a Made-for-Kids build is *approved*.
...
7. ~~Owner decision needed: SHARE parental gate.~~ **Decided and implemented 2026-09-04: SHARE is gated.** The owner chose to put the detail screen's SHARE behind the existing parental gate (Kids Category guideline 1.3 — the share sheet is a door out of the app, and SHARE was always a parent feature per the V2 usage model). See the share/import bullet in "Architecture notes" for the implementation shape.
